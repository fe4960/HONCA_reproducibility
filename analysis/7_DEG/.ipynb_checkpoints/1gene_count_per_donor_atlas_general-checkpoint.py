...
celltype=list(adata.obs["subclass"].unique()) #["Astro_ret","Astro_ON"]
cellnum={}

#list1=f"{main}/7_DEG/genexp_donor_subclass/snRNA_donor_num_cell_Astrocyte" #snRNA_donor_majorclass_num_atlas_cell"
list1=f"{main}/7_DEG/{od}/snRNA_{sampleid}_num_cell_{ct}" #snRNA_donor_majorclass_num_atlas_cell"
...
#		genexp=grouped_obs_cpm(adata=adata_sub, group_key="subclass", layer="raw")
    ...
    #			if cell in adata_sub.obs["subclass"].value_counts():
				if donor not in cellnum:
					cellnum[donor]={}
				cellnum[donor][cell] = adata_sub.obs["subclass"].value_counts()[cell]
				num_tmp=adata_sub.obs["subclass"].value_counts()[cell]
...
list=f"{main}/7_DEG/{od}/snRNA_{sampleid}_num_{ct}" #snRNA_donor_majorclass_num_atlas"
