...
out=paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/genexp_sampleid_subclass_final_new/snRNA_sampleid_num_cell_",c,"_", t, "_ggplot_bar.pdf",width=50)
pdf(out)
print(p)
dev.off()
sub=unique(data$V2)
...
fn=paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/genexp_sampleid_subclass_final_new/snRNA_sampleid_num_cell_",c,"_", t, "_comb_lm_age.txt")
...
res <- df %>%
  ...
  ungroup() %>%
  mutate(FDR = p.adjust(pvalue, method = "BH"))
fn=paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/genexp_sampleid_subclass_final_new/snRNA_sampleid_num_cell_",c,"_", t, "_within_lm_age_range_less30.txt")
write.table(res, file = fn, sep="\t", quote=F)
...
res <- df %>%
  ...
  ungroup() %>%
  mutate(FDR = p.adjust(pvalue, method = "BH"))
fn=paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/genexp_sampleid_subclass_final_new/snRNA_sampleid_num_cell_",c,"_",t, "_within_lm_age_range_less30-70.txt")
write.table(res, file = fn, sep="\t", quote=F)
