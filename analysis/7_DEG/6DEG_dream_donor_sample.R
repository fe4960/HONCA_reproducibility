...
#form <- ~ age  + gender +  race + libsize + (1|batch) + (1|tissue) ### all_new_all_mac_clean
form <- ~ age  + gender +  race + libsize +  (1|tissue) ### all_new_all_mac_clean
...
saveRDS(fitmm,file=paste0(cell,"_fitmm_cpm1.rds"))
...
write.table(res,file=paste0(cell,"_DEG_age_cpm1"),sep="\t",quote=F)
...
write.table(res,file=paste0(cell,"_DEG_gender_cpm1"),sep="\t",quote=F)
...
write.table(res,file=paste0(cell,"_DEG_raceBlack_cpm1"),sep="\t",quote=F)
...
write.table(res,file=paste0(cell,"_DEG_raceHispanic_cpm1"),sep="\t",quote=F)
