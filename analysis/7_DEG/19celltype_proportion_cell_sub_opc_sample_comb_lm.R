...
library(dplyr)
library(ggpubr)
#cell=c("Oligodendrocyte_precursor_cell","Fibroblast", "Astrocyte")
...
res <- df1 %>%
  group_by(ct) %>%
  group_modify(~{
    fit <- lm(prop ~ age + race + gender +  tissue, data = .x)
...
fn=paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/genexp_sampleid_subclass_final_new/snRNA_sampleid_num_cell_",c,"_comb_lm_age.txt")
...
#p=summary(lmr(prop~age+race+gender+(1|tissue),data=df2))
...
p=ggplot(data=df1,aes(x=age_range,y=prop,color=age_range))+geom_boxplot(outlier.shape = NA)+ theme_minimal(base_size = 25)+labs(x = "Age Range", y = "Cell Proportion")+geom_jitter(position=position_jitterdodge(), alpha=0.7)+facet_wrap(~ct, ncol=4, scales="free_y")+
scale_color_manual(
    values = c(
      "<30" = "gold",
      ">=30&<70" = "#74C476",
