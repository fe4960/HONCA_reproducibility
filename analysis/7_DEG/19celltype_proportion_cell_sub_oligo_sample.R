...
p=ggplot(data=df1,aes(x=age,y=prop,color=ct))+geom_point(aes(color=ct,size=6))+geom_smooth(method = "lm", se = F, formula = y ~ x, aes(color=ct))+stat_cor(size=8, aes(color=df1$ct)) + theme_minimal(base_size = 30)+scale_color_brewer(palette="Set3")+ylab("Cell proportion")+xlab("Age (years)")  #+facet_wrap(~ct, ncol=5, scales="free_y")+theme(legend.position="none")
