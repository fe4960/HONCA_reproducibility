...
library(dplyr)
library(ggpubr)
cell=c("Oligodendrocyte_precursor_cell", "Oligodendrocyte", "Astrocyte", "Fibroblast")
...
res <- df1 %>%
  group_by(ct) %>%
  group_modify(~{
    fit <- lm(prop ~ age + race + gender, data = .x)
    coef <- summary(fit)$coefficients["age", ]
    data.frame(
      beta_age = coef["Estimate"],
      se = coef["Std. Error"],
      t = coef["t value"],
      pvalue = coef["Pr(>|t|)"]
    )
...
fn=paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/genexp_donor_subclass_final_new/snRNA_donor_num_cell_",c,"_comb_lm_age.txt")
...
if((n1>=2)&(n2>=2)&(n3>=2)){
p=summary(lm(prop~age+race+gender,data=df2))
print(i)
print(p)
}
}
...
p=ggplot(data=df1,aes(x=age,y=prop,color=ct))+geom_point(aes(color=ct,size=6))+geom_smooth(method = "lm", se = F, formula = y ~ x, aes(color=ct))+stat_cor(size=8, aes(color=df1$ct)) + theme_minimal(base_size = 30)+facet_wrap(~ct, ncol=5, scales="free_y")+theme(legend.position="none")
...
out1=paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/genexp_donor_subclass_final_new/snRNA_donor_num_cell_",c,"_ggplot_lm_oligo100_opc20_1.pdf")
...
out1=paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/genexp_donor_subclass_final_new/snRNA_donor_num_cell_",c,"_ggplot_loess_oligo100_opc20_1.pdf")
...
res <- df %>%
  group_by(ct) %>%
  group_modify(~{
    fit <- lm(prop ~ age_range + race + gender , data = .x)
  ...
  ungroup() %>%
  mutate(FDR = p.adjust(pvalue, method = "BH"))
fn=paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/genexp_donor_subclass_final_new/snRNA_donor_num_cell_",c,"_within_lm_age_range_less30.txt")
write.table(res, file = fn, sep="\t", quote=F)
...
res <- df %>%
  ...
  ungroup() %>%
  mutate(FDR = p.adjust(pvalue, method = "BH"))
fn=paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/genexp_donor_subclass_final_new/snRNA_donor_num_cell_",c,"_within_lm_age_range_less30-70.txt")
write.table(res, file = fn, sep="\t", quote=F)
...
out1=paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/genexp_donor_subclass_final_new/snRNA_donor_num_cell_",c,"_ggplot_box_oligo100_opc20_1.pdf")
