...
fn=paste0(output_PATH, target_tissue,"_6_cell_cluster_dist_y_comb_w1_1.pdf")
p=ggplot(df,aes(x=bin_y,y=prop,fill=harmony_anno))+geom_bar(stat = "identity") +
  ylab("Proportion") + theme_minimal(base_size=20) + scale_fill_manual(values=paired_more)
pdf(fn, width=10)
print(p)
dev.off()


write.table(df, file = paste0(output_PATH, target_tissue,"_6_cell_cluster_dist_y_comb_w1.txt"), sep="\t", quote=F)
#########
