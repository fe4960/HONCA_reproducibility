...
xenium_obj=readRDS(paste0(outdir, "/", t, "_integrated_merFISH.rds"))
...
features <- rownames(xenium_obj)
features <- intersect(features, rownames(tg.ref))


#table(tg.ref$subclass3)


bc=read.table(file=paste0(args[3],"_",t,"_",label),header=T,row.names=1)

xenium_obj=subset(xenium_obj, subset = barcode %in% bc$x)

xenium_obj=xenium_obj[features,]
xenium_obj=xenium_obj


 
 query.counts <- GetAssayData(xenium_obj, assay = "enhanced", slot = "data")
...
#cluster <- as.factor(tg.ref$subclass3)
...
#  saveRDS(RCTD, paste0(output_PATH, label,"_", t, "_subclass2_RCTD_results_nonneuron.rds"))
  ...
  saveRDS(RCTD, paste0(output_PATH, label,"_", t, "_", clu, "_RCTD_results_nonneuron_impute.rds"))
