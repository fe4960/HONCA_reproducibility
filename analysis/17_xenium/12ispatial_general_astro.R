...
options(future.globals.maxSize = 150 * 1024^3)  # 50 GB
...
###############3xenium.merge=merged_obj[,merged_obj$harmony_anno=="Astrocyte"]
############dim(xenium.merge) #

##########3pdf(paste0(outdir, "/", slide_name, "_xenium_umap_annot_astro.pdf"), width = 7, height = 5)
...
slide_name=unique(xenium_data_list[[i]]@meta.data$slide_id)
bc=read.table(file=paste0(bn,"_",slide_name,"_",label),header=T,row.names=1)
...
#xenium_data_list[[i]]
xenium.combined=subset(xenium_data_list[[i]], subset = (barcode %in% bc$x) & (nCount_Xenium >=cut) & (nFeature_Xenium >=fc) )
...
#spRNA <- FindVariableFeatures(spRNA, selection.method = "vst", nfeatures = 2000)
...
meta=read.table("/dfs3b/ruic20_lab/junw42/HCA_ON/data/5_refine_major/scvi/Astrocyte/clean/Astrocyte_subclass_new5_clean_celltype2.obs.gz",header=T,sep=",",row.names=1)
rds="/dfs3b/ruic20_lab/junw42/HCA_ON/data/5_refine_major/scvi/Astrocyte/clean/Astrocyte_subclass_new5_clean_simple.rds"
ref.combined=readRDS(rds)
...
saveRDS(integrated, paste0(outdir, "/", slide_name, "_integrated_astro.rds"))
...
pdf(paste0(outdir, "/", slide_name, "_ispatial_inte_astro.pdf"), width = 8, height = 5)
...
saveRDS(enhancer_expr, paste0(outdir, "/", slide_name, "_enhancer_expr_astro.rds"))
...
saveRDS(integrated_merFISH, paste0(outdir, "/", slide_name, "_integrated_merFISH_astro.rds"))
...
coords <- spRNA@meta.data[, c("x", "y"), drop = FALSE]
coords <- coords[rownames(coords) %in% colnames(integrated_merFISH), , drop = FALSE]
coords <- coords[colnames(integrated_merFISH), , drop = FALSE]

integrated_merFISH@meta.data$x <- coords$x
integrated_merFISH@meta.data$y <- coords$y
...
summary(cor_list)
saveRDS(cor_list, paste0(outdir, "/", slide_name, "cor_list_astro.rds"))
...
pdf(paste0(outdir, "/", slide_name, "_pcc_distribution_astro.pdf"), width = 4, height = 3)
print(p)
dev.off()
...
#saveRDS(hm_df, paste0(outdir, "/", slide_name, "_hm_df_astro.rds"))

#pdf(paste0(outdir, "/", slide_name, "_imputed_marker_heatmap_astro.pdf"), width = 8, height = 6)
#p <- pheatmap(hm_df, scale = "column", cluster_cols = F, cluster_rows = F, color = viridis(n = 10, alpha = 1, begin = 0, end = 1, option = "viridis"), border_color = "black")
#print(p)
#dev.off()
...
saveRDS(integrated_merFISH, paste0(outdir, "/", slide_name, "_integrated_merFISH_clustering_astro.rds"))

pdf(paste0(outdir, "/", slide_name, "_umap_astro.pdf"), width = 8, height = 5)
DimPlot(integrated_merFISH, group.by = "imputed_w1_by_slide", label = T, repel = T)
#DimPlot(integrated_merFISH, group.by = "harmony_anno", label = T, repel = T)
dev.off()
