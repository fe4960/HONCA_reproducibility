...
stopifnot(
  ...
  !anyNA(H_sc),
  !anyNA(H_sp),
  !any(is.infinite(H_sc)),
  !any(is.infinite(H_sp))
)
...
if (is.null(kn$nn.index) || ncol(kn$nn.index) == 0) {
    stop("KNN failed: invalid neighbor matrix")
}
...
#enhancer_expr=na.omit(enhancer_expr)
