...
write.table(dt, file = paste0(output_PATH, target_tissue, "coembed.combined_inte_harmony_p6_harmony_anno_", id[i], "_niches",ni,"_clean_subclass.txt"), sep="\t" , quote=F)

saveRDS(x1, file = paste0(output_PATH, target_tissue, "coembed.combined_inte_harmony_p6_harmony_anno_", id[i], "_niches",ni,"_clean_subclass.rds"))


DefaultBoundary(x1[[fov]]) <-  "segmentation"

pdf(paste0(output_PATH, target_tissue, "_all_annot_majorclass_", id[i], "_niche",ni,"_clean_subclass.pdf"), width = 50, height = 35)
...
pdf(paste0(output_PATH, target_tissue, "_all_annot_majorclass_", id[i], "_heatmap_niche",ni,"_clean_subclass.pdf"), width=5, height=3)
print(p)
dev.off()
