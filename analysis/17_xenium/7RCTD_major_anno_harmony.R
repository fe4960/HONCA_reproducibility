...
xenium.combined=readRDS(paste0(output_PATH1, target_tissue, "coembed.combined_inte_harmony.rds"))   #readRDS( paste0(output_PATH, target_tissue, "_xenium.combined_cutoff",cut,".rds"))
...
pdf(paste0(output_PATH, target_tissue, "_xenium_umap_facet_ref_xenium_combined_inte_harmony_ident_ct_anno_rctd_slide_",slide_id,".pdf"), width = 5, height = 5)
...
DimPlot(x1, reduction = "umap", group.by="rctd1", split.by="ident", label =TRUE) + NoLegend()
