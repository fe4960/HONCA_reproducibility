...
sc.settings.figdir = f"/dfs3b/ruic20_lab/junw42/HCA_ON/data/5_refine_major/scvi/RGC/clean/figures"

celltype="RGC"

indir="/dfs3b/ruic20_lab/junw42/HCA_ON/data/5_refine_major/scvi/RGC/"
mk="sanes_mk"
adata_query=sc.read_h5ad("/dfs3b/ruic20_lab/junw42/HCA_ON/data/4_scvi/RNA/scvi/query_query_latent.h5ad")

adata_query1=sc.read_h5ad(f"/dfs3b/ruic20_lab/junw42/HCA_ON/data/5_refine_major/scvi/RGC/clean/RGC_res_1_clean_res_1.0.h5ad")

adata1=sc.read_h5ad(f"{indir}/clean/{celltype}_hvg10000_epochnone_seurat_v3_rs_1_span_1_subclass_scvi_trg.h5ad")

idx=adata_query1.obs.index.intersection(adata1.obs.index)
adata_query1=adata_query1[idx]

c=["X_scVI","X_umap"]

for c1 in c:
    del adata_query1.obsm[c1]
    adata_query1.obsm[c1]=adata1[idx].obsm[c1]

c=['hvg', 'leiden', 'leiden_colors', 'neighbors', 'umap']

for c1 in c:
    del adata_query1.uns[c1]
    adata_query1.uns[c1]=adata1[idx].uns[c1]

bname=f"{celltype}_subclass_sb"

adata_query1.obs["leiden"]=adata1.obs.loc[idx,"leiden"].astype(str)
...
adata_query1=adata_query1[adata_query1.obs["subclass"].isin(["MG_OFF","MG_ON","HRGC4","PG_ON","PG_OFF","HRGC10","HRGC9","HRGC3","ipRGC"])]

idx=adata_query[adata_query.obs["scANVI_prediction_max_probability"]>0.9].obs.index.intersection(adata_query1.obs.index)
adata_query1.obs.loc[idx,"subclass"]=adata_query.obs.loc[idx,"scANVI_predictions"]

outdir="/dfs3b/ruic20_lab/junw42/HCA_ON/data/5_refine_major/scvi/RGC/clean"
df=adata_query1.obs.groupby(["leiden","subclass"]).size().unstack(fill_value=0)
conf_mat = df / df.sum(axis=1).values[:, np.newaxis]
conf_mat.to_csv(f'{outdir}/{bname}_leiden_RGC_celltype_conf_mat_table_scANVI_h0.9.csv',header=True, index=True)
row_max=conf_mat.max(axis=1)
dominate_name=row_max[row_max > 0.7].index
conf_mat.idxmax(axis=1)
conf_mat.idxmax(axis=1).to_csv(f"{outdir}/{bname}_leiden_RGC_celltype_conf_mat_table_max_scANVI_h0.9.csv")

df=conf_mat.idxmax(axis=1).to_dict()

#adata_query1.obs["celltype"]=adata_query1.obs["leiden"].replace(df)
#adata_query1.obs["subclass"]=adata_query1.obs["celltype"]
df={ "MG_OFF":"RGC_MG_OFF", "MG_ON": "RGC_MG_ON", "HRGC4": "RGC_other", "PG_ON": "RGC_PG_ON", "HRGC9": "RGC_other", "PG_OFF": "RGC_PG_OFF", "HRGC3": "RGC_other"    , "HRGC10": "RGC_other", "ipRGC": "RGC_other"}
adata_query1.obs["subclass"]=adata_query1.obs["subclass"].replace(df)

adata=adata_query1

#adata.obs["subclass"]=adata.obs["leiden"].replace(df)

sc.pl.embedding(adata, basis="X_umap", color=["subclass"],legend_loc="on data", ncols=1,frameon=False,save=f'{bname}_onData.png', palette="tab10")
...
#sc.pl.dotplot(adata, marker, groupby='subclass', dendrogram=False, save=f"{bname}_RGC_marker.png", use_raw=False)
