...
adata_query1=sc.read_h5ad(f"/dfs3b/ruic20_lab/junw42/HCA_ON/data/5_refine_major/scvi/AC/clean/AC_hvg2000_epochnone_seurat_v3_rs_1_clean_rm_scvi_trg.h5ad")

outdir="/dfs3b/ruic20_lab/junw42/HCA_ON/data/5_refine_major/scvi/AC/clean"
...
adata_query1=adata_query1[idx].copy()
...
#########
df2=pd.read_csv("/dfs3b/ruic20_lab/junw42/HCA_ON/data/5_refine_major/scvi/AC/AC_subclass_retina_atlas_ct",header=0,index_col=1)
adata_query1.obs["subclass"]=celltype

adata_query1.obs.loc[idx,"subclass"]=adata_query.obs.loc[idx,"scANVI_predictions"]
adata_query1=adata_query1[adata_query1.obs["subclass"].isin(df2.index)].copy()

#idx=adata_query[adata_query.obs["scANVI_prediction_max_probability"]>0.9].obs.index.intersection(adata_query1.obs.index)
adata_query1.obs["subclass"]=adata_query.obs.loc[adata_query1.obs.index,"scANVI_predictions"]

df=adata_query1.obs.groupby(["leiden","subclass"]).size().unstack(fill_value=0)
conf_mat = df / df.sum(axis=1).values[:, np.newaxis]
conf_mat.to_csv(f'{outdir}/{bname}_leiden_AC_celltype_conf_mat_table_scANVI_h0.9.csv',header=True, index=True)
...
df1=df2.to_dict()["AC_subclass"]
adata_query1.obs["subclass"]=adata_query1.obs["subclass"].replace(df1)
