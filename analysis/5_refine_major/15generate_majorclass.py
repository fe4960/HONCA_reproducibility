...
#fl=["hvg2000_nonsb_epochnone_seurat_v3_rs"]
fl=["hvg10000_sb_epochnone_seurat_v3_rs"]
#obs=pd.read_csv("/dfs3b/ruic20_lab/junw42/HCA_ON/data/5_refine_major/scvi/ON_ONH_clean_new.obs.gz",index_col=0,header=0)

########outdir="/dfs3b/ruic20_lab/junw42/HCA_ON/data/5_refine_major/scvi/Oligodendrocyte/clean/"
########adata1=sc.read_h5ad(f"{outdir}/Oligodendrocyte_hvg10000_epochnone_seurat_v3_rs_0.4_clean_rm4_15_scvi_trg.h5ad")

#######idx=adata1[adata1.obs["leiden"]=="5"].obs.index

########dt={"2": f"OLIGO_SVEP1-hi", "1": f"OLIGO_LRRC7-hi", "0": f"OLIGO_RBFOX1-hi",  "3": f"OLIGO_non_specific", "4": f"OLIGO_NAV2-hi"}

########idx0=adata1[adata1.obs["leiden"]=="0"].obs.index
########idx1=adata1[adata1.obs["leiden"]=="1"].obs.index
########idx2=adata1[adata1.obs["leiden"]=="2"].obs.index
########idx3=adata1[adata1.obs["leiden"]=="3"].obs.index
########idx4=adata1[adata1.obs["leiden"]=="4"].obs.index
...
for f1 in fl:
   # adata=sc.read(f"HCA_ON/data/5_refine_major/scvi/ON_ONH_clean_new.h5ad")
    adata=sc.read(f"HCA_ON/data/5_refine_major/scvi/major/ON_ONH_clean_new.h5ad")
#    del adata.uns["umap"]
...
#    adata.obs["subclass"]=obs.loc[adata.obs.index,"subclass"]
    adata.obs["majorclass1"]=adata.obs["majorclass1"].cat.add_categories([ "Pigmented_cell", "Unknown?", "T_cell", "NK_cell","Mast_cell","Dendritic_cell","B_cell_activated","B_cell", "Immune_RBMS3"])
...
#    sc.pl.embedding(adata, basis="X_umap", color=["subclass"], ncols=1,frameon=False,palette="tab20", save=f"_major_{f1}_1_clean_ON_ONH_new_subclass.png")
    ...
    adata.write("HCA_ON/data/5_refine_major/scvi/major/major_ON_ONH_new_final.h5ad")
    df=adata.obs[['reactionid', 'sampleid', 'donor', 'race', 'gender', 'age', 'tissue','sampleid_legacy','age_year',"globe","pmit"]]
    df1=df.drop_duplicates()
    df1.to_csv("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/genexp_sample_raw_atlas_meta",index=False)

    dir="/dfs3b/ruic20_lab/junw42/HCA_ON/data/5_refine_major/scvi/lattice"

    adata.obs.to_csv(f"HCA_ON/data/5_refine_major/scvi/major/ONONH_new_final.obs.gz",sep="\t")
