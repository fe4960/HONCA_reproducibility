...
for line in Fibroblast #Astrocyte
do
...
#flavor="seurat"
#kp="kp_cl_06"
for kp in  kp_cl_0   #kp_cl_2 kp_cl_0  kp_cl_16 kp_cl_3 kp_cl_4 
do
#kp="kp_cl_2"
flavor="seurat_v3"
#flavor="seurat"
...
#l=${line}_hvg${num}_epoch${epoch}_sd${s}_res${res}_${flavor}_rm_sb
sbatch -p gpu --account=ruic20_lab_gpu --time=0-2 --mem=50GB --gres=gpu:1 --error=${err}/${l}.err --output=${err}/${l}.out  ${main}/HCA_ON/scripts/5_refine_major/6refine_scvi_general.sh $line $s $res $rml $flavor $hvg $epoch $gl $span $kp
done
