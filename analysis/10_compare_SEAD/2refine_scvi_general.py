...
sc.pl.embedding(adata_query, basis="X_umap", color=["sampleid"],  ncols=1,frameon=False,save=f'_{bname}_sampleid.png', palette="tab20")

#sc.pl.embedding(adata_query, basis="X_umap", color=["majorclass"],  ncols=1,frameon=False,save=f'_{bname}_majorclass.png', palette="tab20")

#sc.pl.embedding(adata_query, basis="X_umap", color=["celltype"],  ncols=1,frameon=False,save=f'_{bname}_celltype.png', palette="tab20")


sc.pl.violin(adata_query,keys=["MALAT1", "pct_counts_mt","n_genes_by_counts", "total_counts"], groupby="leiden", rotation=75, save=f"_{bname}_QC.png", multi_panel=True, use_raw=True)
...
#norm_df.to_csv(f'{outdir}/{bname}_leiden_celltype_conf_mat_table.csv',header=True, index=True)
