...
if max_epoch != "none":
    vae.train(max_epochs=int(max_epoch), accelerator="gpu",  datasplitter_kwargs={"drop_last": True})
else :
    vae.train( accelerator="gpu",  datasplitter_kwargs={"drop_last": True})
...
if mk == "none":
    ...
    sc.pl.dotplot(adata_query, markers1, groupby='leiden', dendrogram=False, save=f"_{bname}_marker.png", use_raw=False)
    sc.pl.dotplot(adata_query, known_markers1, groupby='leiden', dendrogram=False, save=f"_{bname}_known_marker.png", use_raw=False)

else:
    lis=pd.read_csv(f'{indir}/{mk}',header=None)
    dt=lis[0].values #.set_index(0).to_dict()[1]
    sc.pl.dotplot(adata_query, dt, groupby='leiden', dendrogram=False, save=f"{bname}_marker_sanes_ct.png", use_raw=False)
...
#dominate_name.to_csv(f'{outdir}/{bname}_{celltype}_scvi_dominate_sample.csv',header=True, index=True)
