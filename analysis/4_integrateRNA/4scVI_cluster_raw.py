...
if obs_rm != "none":
    df=pd.read_csv(obs_rm,header=0,index_col=0)
    idx=adata_query.obs.index.intersection(df.index)
    adata_query=adata_query[idx]
    key=item.split(",")
    for k in key:
#        adata_query.obs["leiden_rm"]=df.loc[idx,k].astype("str")
        adata_query.obs["leiden_rm"]=df[k]
        adata_query.obs.loc[idx,"leiden_rm"]=df.loc[idx,k].astype(str)
#    adata_query.obs["leiden_rm"]=adata_query.obs["leiden"].astype(str)


if obs_kp != "none":
    df=pd.read_csv(obs_kp,header=0,index_col=0)
    idx=adata_query.obs.index.intersection(df.index)
    adata_query=adata_query[idx]
    key=item.split(",")
    for k in key:
#        adata_query.obs.loc[idx,"leiden_kp"]=df.loc[idx,k]
        adata_query.obs["leiden_kp"]=df[k]
        adata_query.obs.loc[idx,"leiden_kp"]=df.loc[idx,k].astype("str")
...
if rm != "none":
    ...
    adata_query=adata_query[-adata_query.obs["leiden_rm"].isin(rmcluster)]

if kp != "none":
    lis=pd.read_csv(f'{dirrm}/{kp}',header=None)
    kpcluster=lis[0].values.astype(str)
    print(kpcluster)
    adata_query=adata_query[adata_query.obs["leiden_kp"].isin(kpcluster)]

adata_query=adata_query[(adata_query.obs["sampleid"]!="MMD_23_17976_ONH_RNA")&(adata_query.obs["sampleid"]!="MMD_23_20181_ONH_RNA")]
...
if flt!=0:
    adata_query=adata_query[adata_query.obs["nFeature_RNA"]>=flt]

####adata_query.write(f'{outdir}/{bname}_scvi_trg_tmp.h5ad')
...
adata_query.obs.to_csv(f'{outdir}/{bname}.obs.gz')
