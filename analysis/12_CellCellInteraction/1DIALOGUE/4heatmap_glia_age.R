...
data=readRDS("/dfs3b/ruic20_lab/junw42/HCA_ON/data/10_DIALOGUE/DIALOGUE//glia_only_noPlot/DLG.full.output_ONONH_glia_only.rds")
mcp=c("MCP1","MCP2","MCP3","MCP4","MCP5","MCP6")
#mcp=c("MCP5")
for(m in mcp){
...
ht=Heatmap(DT1,row_title = paste0(m,"_glia"),  clustering_distance_rows = "euclidean", show_row_names = FALSE, name = paste0(m,"_glia"),  top_annotation = ha, cluster_columns = F, cluster_rows = F,  left_annotation=va)
draw(ht)
dev.off()
}
