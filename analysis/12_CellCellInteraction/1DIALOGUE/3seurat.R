...
rA<- list(r_astro_on=r_astro_on, r_astro_ret = r_astro_ret, r_micro = r_micro, r_macro=r_macro, r_oligo=r_oligo,r_opc=r_opc, r_mg=r_mg)

#rA=c(r_tmp,rA)
#}

param<-DLG.get.param(k = 6,results.dir = "/dfs3b/ruic20_lab/junw42/HCA_ON/data/10_DIALOGUE/DIALOGUE/glia_noPlot/",covar=c("cellQ"), frm= "y ~ (1|donor) + x + cellQ",  n.genes = 200, conf = "cellQ", spatial.flag = F,plot.flag = F) #,pheno = "pathology")

R <- DIALOGUE.run(rA = rA, # list of cell.type objects
                    main = "ONONH_glia",
                    param = param)
saveRDS(R,file="/dfs3b/ruic20_lab/junw42/HCA_ON/data/10_DIALOGUE/DIALOGUE/glia/glia_noPlot.rds")
