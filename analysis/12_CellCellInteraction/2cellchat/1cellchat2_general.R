...
pdf("major_cellcell_interaction_indi.pdf",width=20,height=20)
...
pdf("major_cellcell_interaction_sign_pair_bubble.pdf",width=22,height=40)
...
pdf("major_cellcell_interaction_dominate_sender_receiver_heatmap.pdf",width=20, height=20)
ht1 + ht2
dev.off()
...
nPatterns = 6
pdf("major_cellcell_interaction_MCP_out_heatmap.pdf",width=15,height=20)
...
pdf("major_cellcell_interaction_MCP_in_heatmap.pdf", width=15,height=20)
...
pn=c("BMP",  "WNT", "TGFb", "NOTCH", "TENASCIN" )
for(p in pn){
pathways.show <- c(p)
pdf(paste0("major_cellcell_interaction_",p,"_circle.pdf"))
par(mfrow=c(1,1))
d=netVisual_aggregate(cellchat, signaling = pathways.show, layout = "circle")
#netVisual_aggregate(cellchat, signaling = pathways.show, layout = "chord")
print(d)
dev.off()


pdf(paste0("major_cellcell_interaction_",p,"_chord.pdf"))
par(mfrow=c(1,1))
#netVisual_aggregate(cellchat, signaling = pathways.show, layout = "circle")
d=netVisual_aggregate(cellchat, signaling = pathways.show, layout = "chord")
print(d)
dev.off()


pdf(paste0("major_cellcell_interaction_",p,"_heatmap.pdf"))
par(mfrow=c(1,1))
d=netVisual_heatmap(cellchat, signaling = pathways.show, color.heatmap = "Reds")
print(d)
dev.off()

pdf(paste0("major_cellcell_interaction_",p,"_contr.pdf"))
d=netAnalysis_contribution(cellchat, signaling = pathways.show)
print(d)
dev.off()

pairLR.CXCL <- extractEnrichedLR(cellchat, signaling = pathways.show, geneLR.return = FALSE)

for(i in 1:length(pairLR.CXCL$interaction_name)){
LR.show = pairLR.CXCL[i,]
pdf(paste0("major_cellcell_interaction_",p,"_contr_",i,".pdf"), width=7, height=7)
d=netVisual_individual(cellchat, signaling = pathways.show, pairLR.use = LR.show, layout = "circle",arrow.width = 5)  #vertex.weight=10)
print(d)
dev.off()
}

}
pdf("major_cellcell_interaction_BMP_exp.pdf")
plotGeneExpression(
  cellchat,
...
#plotGeneExpression(cellchat, signaling = pathways.show, enriched.only = FALSE, type = "violin")
dev.off()

#gene=c("BMP5", "BMP4", "BMP7", "BMP2", "BMP1", "BMPR1A", "BMPR2", "BMPR1B", "ACVR1", "BMPR1A", "ACVR2A")

#VlnPlot(cellchat, gene, group.by = "subclass1")
