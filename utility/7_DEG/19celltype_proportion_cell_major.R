...
#p=wilcox.test(df[df$age<50,]$prop, df[df$age>=70,]$prop,alternative="greater")$p.value
...
#####p=ggplot(data=df1,aes(x=age_range,y=prop,color=age_range))+geom_boxplot(outlier.shape = NA)+geom_jitter(width = 0.2)+ theme_minimal(base_size = 25)+scale_color_brewer(palette="Set2")+labs(x = "Age Range", y = "Cell Proportion")
...
p=ggplot(data=df1,aes(x=subclass,y=prop,fill=age_range))+geom_boxplot(outlier.shape = NA)+ theme_minimal(base_size = 25)+scale_fill_manual(values=c("yellow","#018571"))+labs(x = "Age Range", y = "Cell Proportion")+geom_jitter(position=position_jitterdodge(), color="black")+ggtitle(c)+theme(legend.position = "none")

write.table(df1, file=paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/snRNA_sample_num_cell_",c,"_ggplot_overall.txt"), sep="\t", quote=F)
#p=ggplot(data=df1,aes(x=age,y=prop,fill=ct))+geom_point(aes(color=ct,size=6))+geom_smooth(method = "lm", se = F, formula = y ~ x, aes(color=ct))+stat_cor(size=8, aes(color=df$ct)) + theme_minimal(base_size = 25)+scale_color_brewer(palette="Set2")+scale_fill_brewer(palette="Set2")
#out1=paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/snRNA_sample_num_cell_",c,"_ggplot_lm.pdf")
out1=paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/snRNA_sample_num_cell_",c,"_ggplot_overall_ct.pdf")
...
#out1=paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/genexp_donor_subclass_final/snRNA_donor_num_cell_oligo_opc_ggplot_lm_opc50_oligo200.pdf")
out1=paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/genexp_donor_subclass_final/snRNA_donor_num_cell_oligo_opc_ggplot_lm_opc20_oligo200.pdf")
