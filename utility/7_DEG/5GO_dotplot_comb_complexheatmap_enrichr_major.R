...
sam="donor"
#dir1="/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/genexp_donor_subclass_final_new/Oligodendrocyte_subclass_DEG/"
dir1="/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/major_majorclass_DEG/"
df_up_all=NULL
df_down_all=NULL
go_up=NULL
go_down=NULL
...
rm_up=read.table("HCA_ON/scripts/7_DEG/5GO_dotplot_comb_complexheatmap_enrichr_major_rm_up", header=F, sep="\t")
#rm_down=read.table("HCA_ON/scripts/7_DEG/5GO_dotplot_comb_complexheatmap_enrichr_major_rm_down", header=F, sep="\t")
rm_down=c("Mucosal Immune Response")
#rm_down$V1 <- trimws(rm_down$V1, which = "right")
#rm_up$V1 <- trimws(rm_up$V1, which = "right")
lis=read.table("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/ct_list_major4", header=F)

#lis=read.table("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/genexp_donor_subclass_final_new/ct_list_Oligodendrocyte", header=F)
for( i in lis$V1){
  if(i != "Melanocyte"){

    file=paste0(dir1,"/",i,"/",i, "_DEG_GO_age_cpm1_up/", "enrichr_GO_Biological_Process_2023.txt")
if (file.exists(file)) {
...
#    df_down=df[df$NES <0,]
    ...
    df_up1=df_up[c(1:5),]
    print(df_up1$Term)
     if(length(df_up1[df_up1$Adjusted.P.value<=0.1,1]) >0){
    df_up1=df_up1[df_up1$Adjusted.P.value<=0.1,]
...
file=paste0(dir1,"/",i,"/",i, "_DEG_GO_age_cpm1_down/", "enrichr_GO_Biological_Process_2023.txt")
if (file.exists(file)) {
#    df=read.table(file,header=T,sep="\t")
txt <- paste(readLines(file, warn = FALSE), collapse = "\n")
  txt <- gsub("\\\\r", "", txt)
 ...
 #   df=df[!(df$Term %in% rm_down$V1),]
    ...
    df_down1=df_down1[df_down1$Adjusted.P.value<=0.1,]
...
write.table(df_up_all, file=paste0(dir1,"major_all_enrichr_MSigDB_up_heatmap_top10_",sam, "_top_5"), sep="\t", quote=F)
write.table(df_down_all, file=paste0(dir1,"major_all_enrichr_MSigDB_down_heatmap_top10_",sam, "_top_5"), sep="\t", quote=F)
...
out=paste0(dir1,"major_all_enrichr_MSigDB_up_heatmap_top10_", sam, "_top_5.pdf")
...
# Apply the function
short_names <- shorten_names(gsub("\\."," ", colnames(mat_scaled)), width =30)
out=paste0(dir1,"major_all_enrichr_MSigDB_down_heatmap_top10_", sam,"_top_5.pdf")
