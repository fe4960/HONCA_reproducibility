...
geno=read.table("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/genexp_sample_raw_atlas_meta_gsm_race_batch_sampleid_donor", header=T, sep=",") 
df_list <- list()

for(ct in cell){
#    fn <- paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/major_majorclass_sample/exp_",ct,"_logNorm_all")
    fn <- paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/major_majorclass/exp_",ct,"_Norm_all")

    df1 <- read.table(fn, header=TRUE, sep="\t", comment="")

    #df1 <- df1[rownames(df1) == "CDKN2B-AS1", ]
 df1 <- df1[rownames(df1) == "CDKN2B", ]
...
#    df1$pval=data[data$celltype==ct,]$pval_nominal
   ...
   print(ct)
   print(d)
   p=lm(value~age_year+race+gender, data=df1)
   print(summary(p))
}
...
#, label.y=0
#pdf("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/CDKN2B-AS1_DEG_CPM.pdf", width=7.6, height=6)
pdf("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/CDKN2B_DEG_CPM.pdf", width=7.6, height=6)

#pdf("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/RNASEH2B_DEG.pdf", width=7, height=6)
print(p)
dev.off()

write.table(df, file="/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/CDKN2B_DEG_CPM_demo", sep="\t", quote=F)

#write.table(df, file="/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/CDKN2B-AS1_DEG_CPM_demo", sep="\t", quote=F)
