...
library(dplyr)
library(ggpubr)
#cell=c("Oligodendrocyte_precursor_cell")
cell=c("Fibroblast", "Astrocyte")
...
res <- df1 %>%
  group_by(ct) %>%
  group_modify(~{
    fit <- lmer(prop ~ age + race + gender + (1 | tissue), data = .x)
...
fn=paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/genexp_sampleid_subclass_final_new/snRNA_sampleid_num_cell_",c,"_comb_lme_age.txt")
...
#p=summary(lmer(prop~age+race+gender+(1|tissue),data=df2))
...
out1=paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/genexp_sampleid_subclass_final_new/snRNA_sampleid_num_cell_",c,"_", t, "_ggplot_lm_oligo100_opc20_1.pdf")
...
out1=paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/genexp_sampleid_subclass_final_new/snRNA_sampleid_num_cell_",c,"_",t, "_ggplot_loess_oligo100_opc20_1.pdf")
...
res <- df %>%
  group_by(ct) %>%
  group_modify(~{
    fit <- lmer(prop ~ age_range + race + gender + (1 | tissue), data = .x)
  ...
  ungroup() %>%
  mutate(FDR = p.adjust(pvalue, method = "BH"))
fn=paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/genexp_sampleid_subclass_final_new/snRNA_sampleid_num_cell_",c,"_within_lme_age_range_less30.txt")
write.table(res, file = fn, sep="\t", quote=F)
...
res <- df %>%
  ...
  ungroup() %>%
  mutate(FDR = p.adjust(pvalue, method = "BH"))
fn=paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/genexp_sampleid_subclass_final_new/snRNA_sampleid_num_cell_",c,"_within_lme_age_range_less30-70.txt")
write.table(res, file = fn, sep="\t", quote=F)
