...
p=ggplot(data=df,aes(x=ct,y=prop,fill=age_range))+geom_boxplot(outlier.shape = NA) + theme_minimal(base_size = 25)+scale_fill_manual(values=c("gold","#74C476"))+geom_jitter(position=position_jitterdodge())+ggtitle(c)+theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))
##out1=paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/genexp_donor_subclass_final/snRNA_donor_num_cell_",c,"_ggplot_lm.pdf")
