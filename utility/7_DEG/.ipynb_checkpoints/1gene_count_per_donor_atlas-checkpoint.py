...
path=f"{main}/5_refine_major/scvi/lattice/ONONH_all_rawcount.h5ad"
...
list1=f"{main}/7_DEG/snRNA_donor_majorclass_num_atlas_cell"
...
for donor in adata.obs["donor"].value_counts().keys() :
		...
		genexp=grouped_obs_cpm(adata=adata_sub, group_key="majorclass", layer="raw")
		fileout=f'{dir1}/{donor}.txt.gz'
		genexp.to_csv(fileout,sep="\t")
		for cell in celltype :
			if cell in adata_sub.obs["majorclass"].value_counts():
				if donor not in cellnum:
					cellnum[donor]={}
				cellnum[donor][cell] = adata_sub.obs["majorclass"].value_counts()[cell]
				num_tmp=adata_sub.obs["majorclass"].value_counts()[cell]
...
list=f"{main}/7_DEG/snRNA_donor_majorclass_num_atlas"
