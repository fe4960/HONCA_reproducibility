...
p=ggplot(data=data,aes(y=race1,x=V3,fill=V2))+geom_bar(stat="identity",position="fill")+theme_minimal(base_size=25) + theme(legend.position="bottom")+scale_fill_brewer(palette="Paired")+guides(fill = guide_legend(nrow = 3))
out=paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/snRNA_donor_num_cell_race_ggplot_bar.pdf")
pdf(out, width=10, height=5)
print(p)
dev.off()

p=ggplot(data=data,aes(y=gender,x=V3,fill=V2))+geom_bar(stat="identity",position="fill")+theme_minimal(base_size=25) + theme(legend.position="bottom")+scale_fill_brewer(palette="Paired")+guides(fill = guide_legend(nrow = 3))
out=paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/snRNA_donor_num_cell_gender_ggplot_bar.pdf")
pdf(out, width=10, height=4)
print(p)
dev.off()

df=data %>% group_by(V1) %>% summarise(prop=V3/sum(V3),total=sum(V3),V3=V3, ct=V2, age=age, race=race1, gender=gender)
...
write.table(df, file=paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/snRNA_donor_num_cell_race_ggplot_overall.txt"), sep="\t", quote=F)

out1=paste0("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/snRNA_donor_num_cell_race_ggplot_overall.pdf")
