...
lis=read.table("/dfs3b/ruic20_lab/junw42/HCA_ON/data/7_DEG/ct_list_major", header=F)
for( i in lis$V1){
  if(i != "Melanocyte"){
    file=paste0(dir1,"/",i,"/",i, "_DEG_GO_age_cpm1_up/", "enrichr_MSigDB_Hallmark_2020.txt")

#    file=paste0(dir1,"_",i,"_bg/",eng,".txt")
    df=read.table(file,header=T,sep="\t")
#    df=df[!(df$Description %in% rm),]
...
#    df_down=df[df$NES <0,]
    ...
    df_up1=df_up[c(1:10),]
    df_up1=df_up1[df_up1$P.value<=0.05,]
    go_up=c(go_up, df_up1$Term)   
    df_up_all=rbind(df_up_all,df_up)

file=paste0(dir1,"/",i,"/",i, "_DEG_GO_age_cpm1_down/", "enrichr_MSigDB_Hallmark_2020.txt")
    ...
    df_down1=df_down1[df_down1$P.value<=0.05,]
...
write.table(df_up_all, file=paste0(dir1,"_all_enrichr_MSigDB_up_heatmap_top10_",sam), sep="\t", quote=F)
write.table(df_down_all, file=paste0(dir1,"_all_enrichr_MSigDB_down_heatmap_top10_",sam), sep="\t", quote=F)
...
out=paste0(dir1,"_all_enrichr_MSigDB_up_heatmap_top10_", sam, ".pdf")
...
# Apply the function
short_names <- shorten_names(gsub("\\."," ", colnames(mat_scaled)), width =30)
out=paste0(dir1,"_all_enrichr_MSigDB_down_heatmap_top10_", sam,".pdf")
