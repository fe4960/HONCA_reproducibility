...
######pdf(paste0(wd2,"g_", g, "_compareInt_signal_LAMA_astro.pdf"),width=25,height=10)
#######p=plotGeneExpression(cellchat, signaling = "LAMININ", split.by = "datasets", colors.ggplot = T, type = "violin")
#########print(p)
#########dev.off()

order=c("Astro_retina", "Astro_ONH", "Astro_ONHON", "Astro_ON", "OLIGO2_LRRC7+", "OLIGO1", "Oligodendrocyte_precursor_cell", "Microglia", "Mural_cell", "Endothelial_cell", "Fibro_x", "Fibro_dura", "Fibro_arachnoid", "Fibro_pia", "Fibro_RPEchoroid_BMP5")
order1=c("Astro_retina", "Astro_ONH", "Astro_ONHON", "Astro_ON", "OLIGO2_LRRC7+", "OLIGO1", "Oligodendrocyte_precursor_cell",  "Mural_cell", "Endothelial_cell",  "Fibro_dura", "Fibro_arachnoid", "Fibro_pia")

pathways.show <- c("LAMININ")
weight.max <- getMaxWeight(object.list, slot.name = c("netP"), attribute = pathways.show) # control the edge weights across different datasets
pdf(paste0(wd2,"g_", g, "_circle_LAMA_astro_netVis.pdf"), width=12, height=6)

par(mfrow = c(1,2), xpd=TRUE)
#for (i in 1:length(object.list)) {
gg1=netVisual_aggregate(object.list[[1]], signaling = pathways.show, layout = "circle", edge.weight.max = weight.max[1], edge.width.max = 10, signaling.name = paste(pathways.show, names(object.list)[1]), cell.order=order1, sources.use=order1, targets.use=order1,  remove.isolate = TRUE)
gg2=netVisual_aggregate(object.list[[2]], signaling = pathways.show, layout = "circle", edge.weight.max = weight.max[1], edge.width.max = 10, signaling.name = paste(pathways.show, names(object.list)[2]),cell.order=order1,sources.use=order1, targets.use=order1,  remove.isolate = TRUE )
...
ht <- list()
for (i in 1:length(object.list)) {
  ht[[i]] <- netVisual_heatmap(object.list[[i]], signaling = pathways.show, color.heatmap = "Reds",title.name = paste(pathways.show, "signaling ",names(object.list)[i]), remove.isolate = F, sources.use=order1, targets.use=order1)
}
#> Do heatmap based on a single object 
#> 
#> Do heatmap based on a single object
pdf(paste0(wd2,"g_", g, "_circle_LAMA_astro_heatmap.pdf"), width=12, height=6)
...
pdf(paste0(wd2,"g_", g, "_circle_LAMA_astro_netVis1.pdf"), width=12, height=6)

par(mfrow = c(1,2), xpd=TRUE)
#for (i in 1:length(object.list)) {
gg1=netVisual_aggregate(object.list[[1]], signaling = pathways.show, layout = "chord", signaling.name = paste(pathways.show, names(object.list)[1]), sources.use=order1, targets.use=order1, remove.isolate = TRUE, cell.order=order1)
gg2=netVisual_aggregate(object.list[[2]], signaling = pathways.show, layout = "chord", signaling.name = paste(pathways.show, names(object.list)[2]), sources.use=order1, targets.use=order1, remove.isolate = TRUE, cell.order=order1)
...
###############gg1 <- netAnalysis_signalingChanges_scatter(cellchat, idents.use = "Astro_ON")
...
#####################gg4 <- netAnalysis_signalingChanges_scatter(cellchat, idents.use = "Astro_ONHON")
...
################3gg6 <- netAnalysis_signalingChanges_scatter(cellchat, idents.use = "Astro_ONH")
...
##################gg8 <- netAnalysis_signalingChanges_scatter(cellchat, idents.use = "Astro_retina")
