...
#obj <- subset(obj, downsample = 5000, idents = ct )

#obj=subset(obj, subset = celltype %in% c("Fibroblast","Astrocyte","Oligodendrocyte","Mural_cell","Endothelial_cell","Oligodendrocyte_precursor_cell", "Microglia"))
#data.input <- obj1[["RNA"]]@data # normalized data matrix
# For Seurat version >= “5.0.0”, get the normalized data via `seurat_object[["RNA"]]$data`
#labels <- obj1@metadata$majorclass
#meta <- data.frame(labels = labels, row.names = names(labels))

Idents(obj) <- obj@meta.data[[ct]]
...
load("/dfs3b/ruic20_lab/junw42/reference/cellphonedb/cellphonedb-data/data/CellChatDB.human_cellphonedb.rda")
