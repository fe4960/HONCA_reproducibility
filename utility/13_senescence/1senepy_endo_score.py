...
sc.settings.figdir="/dfs3b/ruic20_lab/junw42/HCA_ON/data/5_refine_major/scvi/Endothelial_cell/clean/sene"
hubs = sp.load_hubs(species = 'Human')
hubs.metadata
#Look at hubs within Lung

adata = sc.read_h5ad('/dfs3b/ruic20_lab/junw42/HCA_ON/data/5_refine_major/scvi/lattice/ONONH_Endothelial_cell_rawcount.h5ad')

translator = sp.translator(hub = hubs.hubs, data = adata)


endo_metadata=hubs.metadata[hubs.metadata.cell == 'endothelial cell'] #capitalization matters

hubs.merge_hubs(endo_metadata, new_name = 'endo_merged', calculate_thresh = False)


adata.obs['endo_merged_score'] = sp.score_hub(adata, hubs.hubs["endo_merged"], translator = translator)
...
e = adata.obs.endo_merged_score.mean() #distribution mean
std = adata.obs.endo_merged_score.std() #distribution std
thresh = e + 3*std
thresh



adata.obs['sene_endo'] = adata.obs.endo_merged_score.map(is_putative_sen)

main="/dfs3b/ruic20_lab/junw42/HCA_ON/data/5_refine_major/scvi/Endothelial_cell/clean/sene/"

adata.write(f'{main}/ONONH_Endothelial_cell_rawcount_sene.h5ad')

adata.obs.to_csv(f"{main}/ONONH_Endothelial_cell_rawcount_sene.obs.gz")



sene=["universal_score","universal_thre_score","endo_merged_score"]
...
score=['sene_universal', 'sene_universal_thre', 'sene_endo']
for s in score:
...
# Basic boxplot
    ...
    plt.tight_layout()
    plt.savefig(f"{main}/endo_{s}_all_box_plot.png", dpi=300, bbox_inches="tight")
    plt.show()
    plt.close("all")
...
# Rotate x-axis tick labels if needed
    ...
    plt.tight_layout()
    plt.savefig(f"{main}/endo_{s}_all_lm_plot.png", dpi=300, bbox_inches="tight")
    plt.show()
    plt.close("all")
...
##########
    ...
    plt.tight_layout()
    plt.savefig(f"{main}/endo_{s}_celltype_box_plot.png", dpi=300, bbox_inches="tight")
    plt.show()
    plt.close("all")
...
# Facet plot: one plot per cell type
    ...
    plt.tight_layout()
    plt.savefig(f"{main}/endo_{s}_celltype_lm_plot.png", dpi=300, bbox_inches="tight")
    plt.show()
    plt.close("all")    
