...
#kmean7
################pdf(paste0(output_PATH, target_tissue, "_all_annot_majorclass_", id[i], "_niche_clean_anno_k6_w1.pdf"), width = 50, height = 35)
...
############pdf(paste0(output_PATH, target_tissue, "_all_annot_majorclass_", id[i], "_niche_clean_anno_k16_w1.pdf"), width = 50, height = 35)
...
pdf(paste0(output_PATH, target_tissue, "_all_annot_majorclass_", id[i], "_niche_clean_anno_k10_w1.pdf"), width = 50, height = 35)

###pdf(paste0(output_PATH, target_tissue, "_all_annot_majorclass_", id[i], "_niche_clean_anno_harmony.pdf"), width = 50, height = 35)
p= ImageDimPlot(x1, group.by = "kmean10", border.color=NA, axes=TRUE, border.size = 0, dark.background = T, cols = cols_named, fov=fov)
#cols = "polychrome"
print(p)
dev.off()


pdf(paste0(output_PATH, target_tissue, "_all_annot_majorclass_", id[i], "_niche_clean_anno_k12_w1.pdf"), width = 50, height = 35)
p= ImageDimPlot(x1, group.by = "kmean12", border.color=NA, axes=TRUE, border.size = 0, dark.background = T, cols = "Paired", fov=fov)
print(p)
dev.off()
