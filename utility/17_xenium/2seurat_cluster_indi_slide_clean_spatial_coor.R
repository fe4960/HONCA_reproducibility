...
coords <- as.data.frame(xenium_data_list[[i]]@images$fov$centroids)
df=data.frame(x=coords$x, y=coords$y, row.names=colnames(xenium_data_list[[i]]))
write.csv(df, paste0(output_PATH, "/xenium_spatial_",label,"_",i,".csv"))
