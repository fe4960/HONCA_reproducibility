...
pdf(paste0(output_PATH, target_tissue, "_",label,"_cellsize_rpca_flt",cut,"_fc_",fc,"_",sc,"_slide_",slide_id,"_",idx,".pdf"), width = 15, height = 5)
...
fn=paste0(output_PATH, target_tissue,"_",label,"_cutoff",cut,"_fc_",fc,"_",clu,"_cell_cluster_dist_y_comb_",sc,"_",idx,"_update.pdf")

p=ggplot(df,aes(x=bin_y, y=prop, fill=rctd1  ))+geom_bar(stat = "identity") +
ylab("Proportion") + theme_minimal(base_size = 25) + scale_fill_manual( values= c("purple1", "gold")) #,cale_fill_manual( values= c("purple1", "lightblue",  "yellow",  "pink" ))
