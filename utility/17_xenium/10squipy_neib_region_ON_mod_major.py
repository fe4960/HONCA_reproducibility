...
#    t=i-2
        ...
        dt={"Oligodendrocyte_precursor_cell", "OPC"}
        adata.obs["harmony_anno"]= adata.obs["harmony_anno"].replace(dt)
...
if region == "before_LC" :
    adata_full=adata_full[~adata_full.obs["harmony_anno"].isin(["Rod","Cone","BC","Schwann_cell","RGC","Melanocyte","MG", "HC",  "Oligodendrocyte",  "OPC"])] # "OPC", "Oligodendrocyte", "HC"])]
elif region == "after_LC" :
    adata_full=adata_full[~adata_full.obs["harmony_anno"].isin(["Rod","Cone","BC","Schwann_cell","RGC","Melanocyte","MG", "HC",  "RPE"])]
...
sq.pl.centrality_scores(adata_full, cluster_key="harmony_anno", figsize=(16, 5),   save=f"_centrality_score_ONONH_comb_xenium_merge_cutoff20_major_{region}_clean_major.png" )
...
for cell in adata_full.obs["harmony_anno"].unique():
    sq.pl.co_occurrence(
        ...
        clusters=cell,
        figsize=(10, 10),
        save=f"_{cell}_co_occurrence_ONONH_comb_xenium_merge_cutoff20_major_{region}_clean_major.png"
    )


sq.gr.nhood_enrichment(adata_full, cluster_key="harmony_anno", library_key="library_key")

adata_full.write(f"HCA_ON/data/8_spatial/Xenium/20250620_PP_comb/ONONH_comb/ONONH_combcoembed.combined_inte_squidpy_{region}_clean_major.h5ad")
...
sq.pl.nhood_enrichment(
   ...
   # save=f"_nhood_ONONH_comb_xenium_merge_cutoff20_major_{region}_clean.png",
    save=f"_nhood_ONONH_comb_xenium_merge_cutoff20_major_{region}_clean_st_major.pdf",
