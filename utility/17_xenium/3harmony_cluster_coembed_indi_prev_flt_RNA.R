...
xenium_data_list[[i]]=subset(xenium_data_list[[i]], subset = (barcode %in% bc$x) & (nCount_Xenium >=cut) & (nFeature_Xenium >=25) )
#xenium_data_list[[i]]=subset(xenium_data_list[[i]], subset = barcode %in% bc$x)

xenium_data_list[[i]]=xenium_data_list[[i]][features,]
#Cells(xenium_data_list[[i]])=paste0(t,"_",Cells(xenium_data_list[[i]]))
...
xenium.combined@assays[["RNA"]]=new_assay
Key(xenium.combined[["RNA"]])="RNA_"
xenium.combined@assays[["RNA"]]@data <- xenium.combined@assays[["RNA"]]@counts


DefaultAssay(xenium.combined) <- "RNA"
######
...
merged_obj <- RunHarmony(merged_obj, "slide_id")
...
pdf(paste0(output_PATH, target_tissue, "coembed.combined_harmony_umap_",label, "_rpca_flt",cut,"_",sam,"_slide.pdf"), width = 12, height = 5)
...
saveRDS(merged_obj, paste0(output_PATH, target_tissue, "coembed.combined_inte_harmony_",label, "_rpca_flt",cut,"_",sam,"_slide.rds"))

merged_obj=readRDS(paste0(output_PATH, target_tissue, "coembed.combined_inte_harmony_",label, "_rpca_flt",cut,"_",sam,"_slide.rds"))

pdf(paste0(output_PATH, target_tissue, "_xenium_umap_facet_ref_xenium_combined_inte_harmony_ct_",label,"_rpca_flt",cut,"_",sam,"_slide.pdf"), width = 5, height = 5)
...
pdf(paste0(output_PATH, target_tissue, "_xenium_umap_facet_ref_xenium_combined_inte_harmony_sampleid_",label,"_rpca_flt",cut,"_",sam,"_slide.pdf"), width = 5, height = 5)
...
pdf(paste0(output_PATH, target_tissue, "_xenium_umap_facet_ref_xenium_combined_inte_harmony_slide_",label,"_rpca_flt",cut,"_",sam,"_slide.pdf"), width = 15, height = 5)
DimPlot(merged_obj, reduction = "umap", split.by="slide")+ NoLegend()
dev.off()

table(merged_obj@meta.data[,c("subclass","harmony_clusters_p6")])

tb=table(merged_obj@meta.data[,c("harmony_clusters_p6","subclass")])
pdf(paste0(output_PATH, target_tissue, "_xenium_umap_facet_ref_xenium_combined_inte_harmony_ct_heatmap_",label,"_rpca_flt",cut,"_",sam,"_slide.pdf"), width = 7, height = 7)
heatmap(tb)
dev.off()
...
#####
pdf(paste0(output_PATH, target_tissue, "_xenium_umap_facet_ref_xenium_combined_inte_harmony_ct_",label,"_anno_rpca_flt",cut,"_",sam,"_slide.pdf"), width = 15, height = 5)
DimPlot(merged_obj, reduction = "umap", group.by="harmony_anno",label=F,split.by="slide") #+ NoLegend()
dev.off()

pdf(paste0(output_PATH, target_tissue, "_xenium_umap_facet_ref_xenium_combined_inte_harmony_ct_",label,"_subclass_rpca_flt",cut,"_",sam,"_slide.pdf"), width = 15, height = 5)
DimPlot(merged_obj, reduction = "umap", group.by="subclass",label=F,split.by="slide") #+ NoLegend()
dev.off()

pdf(paste0(output_PATH, target_tissue, "_xenium_umap_facet_ref_xenium_combined_inte_harmony_ct_",label,"_count_rpca_flt",cut,"_",sam,"_slide.pdf"), width = 15, height = 5)
...
pdf(paste0(output_PATH, target_tissue, "_xenium_umap_facet_ref_xenium_combined_inte_harmony_ct_",label,"_feature_rpca_flt",cut,"_",sam,"_slide.pdf"), width = 15, height = 5)
#FeaturePlot(merged_obj, reduction = "umap", features="counts",split.by="slide") #+ NoLegend()
FeaturePlot(merged_obj[,merged_obj@meta.data$slide!="snRNA"], reduction = "umap", features="nFeature_Xenium",split.by="slide")
dev.off()

pdf(paste0(output_PATH, target_tissue, "_xenium_umap_facet_ref_xenium_combined_inte_harmony_ct_",label,"_cellsize_rpca_flt",cut,"_",sam,"_slide.pdf"), width = 15, height = 5)
#FeaturePlot(merged_obj, reduction = "umap", features="counts",split.by="slide") #+ NoLegend()
