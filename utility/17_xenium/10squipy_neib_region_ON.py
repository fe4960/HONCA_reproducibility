...
if region == "before_LC" :
    adata_full=adata_full[~adata_full.obs["harmony_anno"].isin(["Rod","Cone","BC","Schwann_cell","RGC","Melanocyte","MG", "HC", "Astrocyte", "Oligodendrocyte"])] # "OPC", "Oligodendrocyte", "HC"])]
else:
    adata_full=adata_full[~adata_full.obs["harmony_anno"].isin(["Rod","Cone","BC","Schwann_cell","RGC","Melanocyte","MG", "HC", "Astrocyte", "Oligodendrocyte"])]
...
sq.pl.centrality_scores(adata_full, cluster_key="harmony_anno", figsize=(16, 5),   save=f"_centrality_score_ONONH_comb_xenium_merge_cutoff20_major_{region}_clean.png" )
...
for cell in adata_full.obs["harmony_anno"].unique():
    sq.pl.co_occurrence(
        ...
        clusters=cell,
        figsize=(10, 10),
        save=f"_{cell}_co_occurrence_ONONH_comb_xenium_merge_cutoff20_major_{region}_clean.png"
    )


sq.gr.nhood_enrichment(adata_full, cluster_key="harmony_anno", library_key="library_key")

adata_full.write(f"HCA_ON/data/8_spatial/Xenium/20250620_PP_comb/ONONH_comb/ONONH_combcoembed.combined_inte_squidpy_{region}_clean.h5ad")
