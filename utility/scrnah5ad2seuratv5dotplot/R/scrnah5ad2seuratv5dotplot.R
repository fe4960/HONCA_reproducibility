...
x=h5ad2seuratv5(infile, bpcdir, catobs=groupby)
...
x=CreateSeuratObject(
	counts=LayerData(x, assay='RNA', layer=if (scale) 'scale.data' else if (norm) 'data' else 'counts'),
	meta.data=subset(
		x@meta.data,
