...
sc.pp.highly_variable_genes(
        adata, flavor="seurat_v3", n_top_genes=ntop, subset=True
...
adata.obsm["age"] = np.asarray([[x] for x in adata.obs["age"]])
...
local_correlations.to_csv(
    output_path + "/" + label + f"_hs_local_correlations_years_hvg_{ntop}.csv"
)
module_scores.to_csv(
    output_path + "/" + label + f"_hs_module_scores_years_hvg_{ntop}.csv"
)
joblib.dump(
    hs,
    output_path + "/" + label + f"_hs.create_modules_years_hvg_{ntop}.pkl",
)
adata.write(output_path +  "/" + label +  f"_hs.adata_years_hvg_{ntop}.h5ad"
)
