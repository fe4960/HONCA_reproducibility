...
adata1=sc.read_h5ad(f"{outdir}/{celltype}_hvg10000_epochnone_seurat_rs_0.6_rmrm_rs1_0_sb_scvi_trg.h5ad")
...
obs=pd.read_csv(f"{outdir}/{celltype}_hvg10000_epochnone_seurat_rs_0.4_rmrm_rs1_0_sb.obs.gz", header=0, index_col=0)
adata.obs["leiden"]=obs.loc[idx,"leiden"].astype(str)
bname=f"{celltype}_subclass_sb_clean_rmRPE"
...
adata.obs["subclass"]="Macrophage"
#idx=adata.obs.loc[adata.obs["leiden"]=="8"].index
#adata.obs.loc[idx,"subclass"]="Macrophage_VCAN+"
#idx=adata.obs.loc[adata.obs["leiden"]=="9"].index
#adata.obs.loc[idx,"subclass"]="Macrophage_RORA+"
#idx=adata.obs.loc[adata.obs["leiden"]=="10"].index
#adata.obs.loc[idx,"subclass"]="Macrophage_RRM2+"
#idx=adata.obs.loc[adata.obs["leiden"]=="11"].index
...
dt={"0": f"Macro_Res_Activated", "1": f"Macro_Res", "2": f"Macro_Monocyte_Derived_Inflammatory", "3": f"Macro_Res_Proliferating", "4": f"Macro_Res_Remodeling", "5" : f"Macro_Res_Stress_Response", "6": f"Macro_Monocyte_Derived_APC"}

#dt={"0": f"Macro_SPP1+", "1": f"Macro", "2": f"Macro_VCAN+", "3": f"Macro_EZH2", "4": f"Macro_RBMS3", "5" : f"Macro_JUN+", "6": f"Macro_HLA-DPB1+"}
