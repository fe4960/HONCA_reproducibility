...
mk="sanes_mk"
adata=sc.read_h5ad(f"{outdir}/{celltype}_subclass_sb.h5ad")
adata1=sc.read_h5ad(f"{outdir}/{celltype}_hvg10000_epochnone_seurat_rs_0.6_rmrm_micro_res0.7_sb_scvi_trg.h5ad")
...
c=['hvg', 'leiden',  'neighbors', 'umap']

for c1 in c:
    del adata.uns[c1]
    adata.uns[c1]=adata1[idx].uns[c1]

#adata=sc.read_h5ad(f"{outdir}/{celltype}_hvg2000_epochnone_seurat_v3_rs_0.4_clean_rm_rm_scvi_trg.h5ad")
bname=f"{celltype}_subclass_sb_clean"


#adata.obs["leiden1"]="Microglia"

adata.obs["subclass"]="Microglia"
sc.settings.figdir = f"{outdir}/figures"

obs=pd.read_csv(f"{outdir}/{celltype}_hvg10000_epochnone_seurat_rs_0.4_rmrm_micro_res0.7_sb.obs.gz",header=0, index_col=0)

adata.obs["leiden"]=obs.loc[idx, "leiden"].astype(str)
...
#obs=pd.read_csv(f"{outdir}/{celltype}_res_0.2_res_0.2.obs.gz",header=0, index_col=0)
...
#adata=adata[adata.obs["leiden1"]!=3]
dt={"0": f"Micro_Homeostatic", "1": f"Micro_Activated", "3": f"Micro_Proliferating", "4": f"Micro_Remodeling", "2": f"Micro_Stress_Response", "5": f"Micro_NFkB_inflammatory" }

#dt={"0": f"Micro", "1": f"Micro_SLC2A3high", "3": f"Micro_CENPP+", "4": f"Micro_RBMS3+", "2": f"Micro_FTL+", "5": f"Micro_NFKB1+" }
