...
bname=f"{celltype}_subclass_clean"
#adata.obs["leiden1"]="Mural_cell"

adata.obs["subclass"]="Mural_cell"
sc.settings.figdir = f"{outdir}/figures"
#HCA_ON/data/5_refine_major/scvi/Mural_cell/clean/Mural_cell_hvg10000_epochnone_seurat_rs_0.6_rmrm_clu1_sb.obs.gz
#obs=pd.read_csv(f"{outdir}/{celltype}_res_0.6_clean_res_0.6.obs.gz",header=0, index_col=0)
obs=pd.read_csv(f"{outdir}/{celltype}_hvg10000_epochnone_seurat_rs_0.6_rmrm_clu1_sb.obs.gz",header=0, index_col=0)
...
#dt={"0": f"vascularSMC_SORBS2+", "1": f"Pericyte_PDE7B+", "2": f"Pericyte_CFH+" ,  "3": f"vascularSMC_SLC7A2+", "5": f"vascularSMC_SORBS2+"}
dt={"1": f"vascularSMC_UNC13C+", "0": f"Pericyte_PDE7B+", "2": f"Pericyte_CLSTN2+" ,  "3": f"vascularSMC_ZBTB7C+"}
