...
bname=f"{celltype}_subclass_LC6"
...
idx=obs.loc[obs["leiden"]==6].index
...
celltype1="Fibro"
dt={"0": f"{celltype1}_arachnoid", "1": f"{celltype1}_pia", "2": f"{celltype1}_sclera", "3": f"{celltype1}_dura", "4": f"{celltype1}_RPEchoroid", "5" : f"{celltype1}_x"}
...
adata.obs["subclass"]=adata.obs["subclass"].cat.add_categories(["Fibro_LC"])

adata.obs.loc[idx,"subclass"]=f"{celltype1}_LC"
...
mk=["ACTA2"]#,"GFAP"]
sc.pl.dotplot(adata2,mk,groupby="subclass", save="Fibroblast_subclass_LC6_ACTA2.png")
