...
df=pd.DataFrame(adata.obs["majorclass"].value_counts())
...
plt.show()
fig.savefig(f"{dir}/HCA_ON/data/5_refine_major/scvi/major/figures/{bname}.png", dpi=300)

df.to_csv(f"{dir}/HCA_ON/data/5_refine_major/scvi/major/figures/{bname}.csv" )

#sc.pl.embedding(adata, basis="X_umap", color=["majorclass"], ncols=1,frameon=False, save=f"_major_{f1}_1_clean_ON_ONH_new_majorclass_onData.png", palette="tab20", legend_loc="on data")
...
df=adata.obs[["nCount_RNA","majorclass","sampleid","tissue","source"]].groupby(["tissue","source","sampleid","majorclass"]).count()

df.to_csv(f"{dir}/HCA_ON/data/5_refine_major/scvi/major/figures/{bname}_majorclass_proportion_sample.csv")
...
for s1 in s:
    for t1 in t:
        df=obs.loc[(obs["source"]==s1)&(obs["tissue"]==t1),["nCount_RNA","majorclass","sampleid","tissue","source"]].groupby(["tissue","source","sampleid","majorclass"]).count()
        df.to_csv(f"{dir}/HCA_ON/data/5_refine_major/scvi/major/figures/{bname}_majorclass_proportion_sample_{t1}_{s1}.csv")
